    ...
    <div class="content"> 
		...
		<div class="page-title">	
			...
			<h4>
				...
				<?php }?>
				<a class="pull-right btn btn-primary btn-xs btn-mini" href="<?php echo site_url('message/index/'.$quote->id);?>">View Messages</a>
			</h4>
								...
								<table class="no-more-tables general" border="0">
									<tr>
										<td>
									      ...
									      <!-- <a target="_blank" href="<?php echo site_url('quote/viewquote/'.$quote->id); ?>">Original</a><br> -->									       <?php if(isset($bid->id)) { $quotearr = explode(".",$bid->quotenum);  ?> <a href="<?php echo site_url('quote/viewbid/'.$bid->id);?>">show supplier quote &nbsp;<?php echo $quotearr[0].".000"; ?></a><br><?php } ?>
									      <?php  if(isset($revisionno)) { $quotearr = explode(".",$bid->quotenum);  for($i=2;$i<=$revisionno;$i++) { ?><a href="<?php echo site_url('quote/viewbids/'.$bid->id.'/'.$i);?>">show supplier quote &nbsp;<?php echo $quotearr[0]."."; printf('%03d',($i-1)); ?></a><br><?php } } ?></td></tr>
									      <td colspan="2">   
									      	<br/><br/>
