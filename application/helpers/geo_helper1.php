...
function get_my_address() {
    $CI = &get_instance();
    if ($CI->session->userdata('navigator_lat') && $CI->session->userdata('navigator_lng')) {
        ...
        $details->country = get_nearest_city($geo_coords[0], $geo_coords[1], $return = "country");
    } else {
    ...
    if (strtolower($data['status']) == "ok") {
        ...
        if (is_array($info)) {
            foreach ($info as $gps_item) {
                if (in_array($gps_item['types'][0], array('postal_code', 'postal_code_prefix'))) {
