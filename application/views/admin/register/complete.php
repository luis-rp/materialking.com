		...
		<?php echo form_open('admin/register/savecomplete') . "\n"; ?>
			...
			<fieldset>
				<div class="fields">
					...
					<input type="text" name="city" placeholder="City" id="city" value="<?php if(isset($user->city) && $user->city!="") echo $user->city;?>" tabindex="5" required><br/>
					<input type="text" name="state" placeholder="State" id="state" value="<?php if(isset($user->state) && $user->state!="") echo $user->state;?>" tabindex="6" required><br/>
					<input type="text" name="zip" placeholder="Zip" id="zip" value="<?php if(isset($user->zip) && $user->zip!="") echo $user->zip;?>" tabindex="7"
					onchange="IsValidZip(this.form.zip.value)" required>
					 <textarea name="street" tabindex="8" placeholder="Street Address" id="street" required rows="4" style="margin-top:3px; width: 96%" required><?php if(isset($user->street) && $user->street!="") echo $user->street;?></textarea>
				</div>
					<input type="hidden" name="hiddenuserid" id="hiddenuserid" value="<?php if(isset($user->id) && $user->id!="") echo $user->id; ?>"/>
				<button type="submit" class="btn btn-primary btn-block" tabindex="9">Continue</button>
			</fieldset>
		<?php echo form_close(); ?>
