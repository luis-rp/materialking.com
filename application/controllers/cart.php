 ...
 $isnetwork=$this->db->get_where('network',array('company'=>$item['company'],'purchasingadmin'=>$this->session->userdata('site_loggedin')->id))->row();
	...
	public function ccpost()
	{
		...
		foreach($cart as $ci)
		{
		 ...
		 $tax = round($totalprice*$settings->taxpercent/100,2);
		...
		$totalprice = $totalprice+ $totalshipping+ $tax + $comission + ($companycount*0.25) + 0.8;
...
";
    		...
    		//	$itemdetails->itemorgname = $orgitem->itemname;
				$item['itemdetails'] = $itemdetails;
